...
#define MB_PDU_READ_OUT_BYTECNT_OFFSET  (MB_PDU_DATA_OFFSET + 1)
#define MB_PDU_READ_OUT_DATA_OFFSET		(MB_PDU_DATA_OFFSET + 2)

#define MB_PDU_WRITE_MUL_ADDR_OFFSET	(MB_PDU_DATA_OFFSET + 0)
...
static void mb_slave_receive_event(MB_RTU_HandleTypeDef *mb_rtu, uint8_t *pdu_buf, uint8_t pdu_len);
...
static MB_Exception mb_slave_func_read_input_registers(uint8_t *pdu_buf, uint8_t *pdu_len)
{
	...
	*pdu_len = MB_PDU_DATA_OFFSET + reg_count * 2;
